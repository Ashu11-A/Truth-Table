...
import './index.js'
import { isError, Structure, Table, TableType } from './index.js'
...
(() => {
  if (
    ...
    !Object.keys(packageJ.bin).includes(basename(process.argv[1]))
  )
    return
  ...
  new Terminal([
    ...
    },
    {
      ...
      command: 'display',
      description:
        'How the data will be displayed in the table, supports: boolean, number',
      rank: 1,
      hasString: true,
      async function(content) {
        if (!(content === 'number' || content === 'boolean'))
          throw new Error('Display command can only be “boolean” or number')
    ...
    },
    {
      ...
      hasString: true,
      async function(content) {
        if (!(content === 'csv' || content === 'markdown'))
          throw new Error('Type command can only be csv or text')
        Object.assign(TableSettings, {
          type: content,
        } satisfies Partial<TableType>)
      },
    },
    {
      ...
      async function(content) {
        ...
        const ast = new Analyzer(content).parse()
